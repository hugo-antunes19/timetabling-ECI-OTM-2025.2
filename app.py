...
def gerar_grade():
    ...
    """
    # --- NOVA LINHA: Captura a escolha do próximo período ---
    proximo_periodo_tipo = request.form.get('proximo_periodo', 'impar') # Default para 'impar'
    print(f"Tipo do próximo período letivo: {proximo_periodo_tipo}")
    ...
    # (O resto do código que calcula os créditos restantes continua igual...)
    try:
        with open(CAMINHO_DISCIPLINAS, 'r', encoding='utf-8') as f:
    ...
    if grade_result:
        ...
        for s, disciplinas_do_semestre_str in grade_result.items():
            if not disciplinas_do_semestre_str: continue
            ...
            for disciplina_str in disciplinas_do_semestre_str:
                ...
                if match:
                    ...
                    for horario_completo in horarios:
                        ...
                        if len(parts) == 3:
                            dia, horario_key = parts[0], f"{parts[1]}-{parts[2]}"
