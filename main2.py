...
def gerar_grade_otimizada():
    ...
    for d_id in disciplinas:
        ...
        for t_id in turmas_por_disciplina[d_id]:
            for s in range(1, NUM_SEMESTRES + 1):
                ...
                # REATIVANDO A RESTRIÇÃO DE PERÍODO
    ...
    # Reintroduzindo as variáveis de semestre (modelo robusto)
    ...
    # R1: Cada disciplina deve ser cursada exatamente uma vez.
    for d_id in disciplinas:
        turmas_validas = [var for key, var in alocacao.items() if key[0] == d_id]
        model.AddExactlyOne(turmas_validas)

    # R2 (Ligação): Ligar a variável 'semestre_da_disciplina' com as variáveis 'alocacao'.
    for d_id in disciplinas:
        for s in range(1, NUM_SEMESTRES + 1):
    ...
    # R3 (Pré-requisitos): Versão robusta e correta.
